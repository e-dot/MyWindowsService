...
SET SERVICE_START_MODE=%~6
SET SERVICE_AUTOSTART=%~7
...
IF "%SERVICE_START_MODE%" == "" SET SERVICE_START_MODE=auto
...
SC CREATE "%SERVICE_NAME%" binPath= "%SERVICE_EXE%"%SERVICE_OPTIONS% DisplayName= "%SERVICE_LABEL%" start= %SERVICE_START_MODE% || GOTO ERROR
...
:USAGE
ECHO [%TIME%] Usage : %~nx0 SERVICE_PASSWORD [SERVICE_LOGIN=%USERNAME%] [SERVICE_NAME=.\*.exe] [SERVICE_LABEL=SERVICE_NAME] [SERVICE_LOG_FOLDER=SERVICE_PATH] [SERVICE_START_MODE=auto^|delayed-auto] [SERVICE_AUTOSTART=NO^|AUTOSTART]>&2
EXIT /B 0
